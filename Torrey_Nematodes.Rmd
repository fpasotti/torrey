...
##### some extra visualisation of Nematode genera and families #######
```{r stacked barplot , echo=FALSE}

## stacked barplot can help visualise the diversity even when not significantly different and support other indices such as the 
#diversity indices or the maturity index 
library(dplyr)
library(ggplot2) 
...
cols_2 = c('gray', 'coral', 'chartreuse3','cornflowerblue')
...
#plot1 <- plot + theme(text = element_text(size=25))# this if I want the legend in

plot1 <- plot +   theme(legend.position = "none", text=element_text(size=25)) # this only if I want to then plot two plots together 

ggsave("plot.png",width = 70, height = 40, units = "cm")

legend <- get_legend(p) # to have the legend for later
...
plot2 <- ggplot()+
  geom_point(data=MI.2, aes(x = Station, y = MI, colour = factors$Habitat, shape= factors$Habitat),size=3 )+
  theme_classic()+
...
```{r merge two plots}
library(ggpubr)
plot3 <- ggarrange(plot2,plot1,legend,nrow=3,ncol=1)

print(plot3)

ggsave("plot3.png",width = 70, height = 40, units = "cm")
```
...
c_p <- Nema [, c(-1, -3)] #I first create a df that only has genus (col1) and c_p values (col2) and samples with counts (cols 3:17)
c_p <-na.omit(c_p)# I remove all NAs since I cannot calculate or ID properly
library(dplyr)
relative_abundance <- c_p[, 3:17] / colSums(c_p[, 3:17], na.rm = TRUE) # I calculate the relative abundance of the genera
...
c_p.3 <- c_p.3 %>%
 mutate(
    Strategy = case_when(
      ...
      c_p == "2" ~ "Coloniser",
       c_p == "3" ~ "Intermediate",
       c_p == "4" ~ "Stress sensitive",
     c_p == "5" ~ "Extreme persisters"))
